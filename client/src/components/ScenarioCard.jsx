...
function ScenarioCard(props) {
  ...
  const { isSelected, handleSelect } = useScenarioContext()
    ...
    (event) => {
      ...
      const config = {
        ...
        body: JSON.stringify({
          ...
          region: formValues.region, // include region field
        }),
      }
      ...
      <span className={styles.form_button}>
        ...
        >
          <Conditionally
            when={downloadUrl === ''}
            otherwise={(
              // Instead of rendering a download link, show an "Open Terminal" button.
              <button
                onClick={() => window.open('/terminal', '_blank', 'width=800,height=600')}
...
export { ScenarioCard as default }
